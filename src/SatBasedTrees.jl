...
"""
function completeTree(t::Dict{Int,Node}, max_depth::Int)
    ...
    for i = (2^max_depth-1):-1:1
        t[i] = Node(i, false, [t[2*i], t[2*i+1]])
    end
end
...
"""
function computeLeftAncestors(t::Dict{Int,Node}, leaves)
...
"""
function computeRightAncestors(t::Dict{Int,Node}, leaves)
        ...
        if haskey(beta, node_t)
            ...
            if !split_found
                ...
                if max_left_value != -Inf
                    ...
                    println("  Nodo $node_t -> Soglia $(alpha[node_t]) (fallback)")
                end
            end
        end
    end
...
function main()
    ...
    #=
    if solution !== nothing
        println("Albero ottimale trovato!")

        """
        # prova per ultima funzione scritta
        ...
        tree = printTreeSAT(alpha, beta, theta)
        println("\nStruttura dell'albero decisionale ottimale:")
        print_tree(tree[1])
    end
    =#
